...
markers <- 
    ...
    mutate(id = if_else(str_length(id) == 1, paste0("0",id), id),
           picture = if_else(category == 1, 
                             "Negative", 
                             "Neutral", 
                             missing = NA_character_) %>% 
                     as_factor(levels = c("Neutral", "Negative"))) %>% 
    # Drop training pictures
    drop_na(block) %>% 
    select(-category)

scr_data <- read_csv("analysis_data/scr_era.csv")
hr_data <- read_csv("all_data/hr/hr_linear.csv")
...
It seems like all scr data can be matched to markers.
...
Moreover, the complete session of emot is missing for 04, 42; and recall is missing for 33, 34.
```{r}
...
hrd <-
    ...
    summarise(hrd = min(hr_change, 
                        na.rm = TRUE),
              hrd_std = min(hr_change_std, 
                            na.rm = TRUE)) %>% 
    ungroup() %>% 
    mutate(novelty = case_when(
        session == "recall" & pic_set == "A" & stimulus %in% set_a ~ "Old",
...
hrd_sum <-
    hrd %>% 
    group_by(id, session, block, group, picture, novelty) %>% 
    summarise(hrd = mean(hrd_std, na.rm = TRUE)) %>% 
    ungroup()

# Using Zuur method to select random structure
# Simple linear regression to justify lmer
hrd_lm <- lm(hrd ~ group * block * picture, data = hrd_sum)
# Random intercept
hrd_i <- lmer(hrd ~ group * block * picture + (1 | id) , data = hrd_sum)
# Random intercept and splope
hrd_is <- lmer(hrd ~ group * block * picture + (picture * block | id), data = hrd_sum)
